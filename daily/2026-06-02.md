...
## 晚间转折：hackathon 规则落定 + 方向重构

拿到比赛规则页（https://casualhackathon.com/hackathons/cmpsjubkg0003p80kxuzrdyjy），三个冲击：

1. **没有 AI Security 赛道**：只有 Cobo（Agentic Economy × CAW）和 Z.AI（Long-Horizon Task）两个赞助赛道。learning-plan 里那 6 个 track 是 Handbook 探索章节，不是这场比赛的赛道。
2. **Cobo 硬规则撞设计哲学**：要求 Agent **真实资金执行**（经 CAW），"仅停留在流程设计层面不收"。旧 Auditor 的"不真转钱、只审查"会被判不合格。
3. **必须可运行 Demo**：又一次确认 0 代码的硬伤。

**重构决定（不是放弃）**：做**一个**项目——长程可审计 Agent，主攻 Cobo，GLM-5.1 当脑保留 Z.AI 选择权。核心洞察：**长程自主跑在可逆的活上，human gate 只卡不可逆那一步**；Auditor 升级为"让 Agent 敢长程自主的前提"。单干 + 11 天 → 不做两个项目（会得两个半成品），而是做一个两赛道都够资格的项目，提交时挑分数高的赛道。旧 threat_model/authorization/FSM/架构几乎全平移（deposit→CAW payment）。详见 hackathon/ideation.md 重写版。

**承重墙**：`用户意图 → Agent(GLM-5.1)自主准备一笔 CAW 操作 → Auditor 把关 → 用户放行 → CAW 测试网真执行 → 回放日志`。今晚 20:00 Open Day 确认 CAW 能力后定稿场景（倾向 Agent Resource Procurement）。
